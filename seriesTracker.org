...
Get all series updated since the last known date (date of the latest episode watched).
Query all followed series that are updated and add all new episodes.

#+begin_src elisp
(defun tvdb-update ()
  "Query all updated series since the day of the latest watched episode.
Keep only series that are followed.
Update new episodes."

  (let* ((updatedSeries (-some->> tvdb--data
                          (--map (->> (alist-get 'episodes it)
                                    (--filter (equal (alist-get 'watched it) t))
                                    (array-pull 'firstAired)))
                          (-flatten)
                          (-map 'date-to-epoch)
                          -max
                          epoch-to-datetime
